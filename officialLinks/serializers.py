from rest_framework import serializers
from .models import OfficialLink
import base64

class OfficialLinkSerializer(serializers.ModelSerializer):
    bg_image = serializers.SerializerMethodField()
    image_upload = serializers.CharField(write_only=True, required=False)

    class Meta:
        model = OfficialLink
        fields = ['id', 'title', 'details', 'links', 'bg_image', 'image_upload']

    def get_bg_image(self, obj):
        if obj.bg_image:
            return base64.b64encode(obj.bg_image).decode('utf-8')
        return None

    def create(self, validated_data):
        image_data = validated_data.pop('image_upload', None)
        if image_data:
            validated_data['bg_image'] = base64.b64decode(image_data)
        return super().create(validated_data)

    def update(self, instance, validated_data):
        image_data = validated_data.pop('image_upload', None)
        if image_data:
            instance.bg_image = base64.b64decode(image_data)
        return super().update(instance, validated_data)
